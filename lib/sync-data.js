...
function syncData(username, facebookUser) {
	...
	return userDb.child('lastUpdate').once('value', function(data) {
		console.log('Syncing updates in last',moment(data.val()).format("HH:mm:ss.SSS"))
		var lastUpdate = data.val()
		tinder.getUpdates(Date.now() - lastUpdate || 1000*60*60*14, function updateCallback(err, updates) {
			console.log('Received new updates')
			if (err) return console.error(err)
			updates.matches.forEach(function(match) {
				if (moment(match.last_activity_date).isAfter(moment(lastUpdate))) {
					match.viewed = false
				}
				userDb.child('matches').child(match._id).update(match, function() {
					console.log('Added updated match')
				})
			})
			userDb.child('blocks').set(updates.blocks)
...
db.authWithCustomToken(process.env.TINDER_FIREBASE_TOKEN, function(error, result) {
	...
	return db.child('userList').once('value', function(s) {
		var users = s.val()
