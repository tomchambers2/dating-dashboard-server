...
var TinderClient = function(userId, token) {
	...
	function _sortUpdates(updates, callback) {
		console.log('all udpates',updates)
		updates.matches = _.sortByOrder(updates.matches, 'last_activity_date', false)
	...
	function _sendMessage(matchId, message, callback) {
		console.log('sending message',matchId, message)
		tinder.send_message(matchId, message, function(err, res, data) {
			if (err) return callback(err)
			if (res.statusCode!=200) return callback(res.statusCode || data.error)
	...
	this.getUpdates = function(since, callback) {
		async.waterfall([
			...
			if (err) return callback(err)
			console.log('returing',allUpdates)
			if (err) return callback(err)
			callback(null, allUpdates)
		})
	}
	...
	this.getChatPartnerDetails = function(matchId, callback) {
		async.waterfall([
			...
			if (err) return callback(err)
			if (!match) return callback("This user could not be found in history",matchId)
			currentPartner = match._id
			callback(null, match)
		})
	}

	this.like = function(id, cb) {
		tinder.like(id, cb)
	}

	this.dislike = function(id, cb) {
		tinder.like(id, cb)
	}
